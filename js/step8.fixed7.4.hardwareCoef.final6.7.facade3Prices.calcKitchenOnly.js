...
(function () {
    ...
    function calcFacadeOperations(facades) {
        return facades.map(facade => {
            const { type, hasHandle, hasGlass, isDrawer } = facade;

            const cutOperations = 4; // 4 різи на фасад
            const edgeOperations = 4; // 4 сторони
            const drillOperations = (facade.hinges || 0) * 1 + (hasHandle ? 2 : 0);
            const drawerOperations = isDrawer ? 1 : 0;
    ...
    // Приклад фасадів (можна вставити цю частину в обробку даних фасадів)
    const facades = [
        { type: 'base', hasHandle: true, hasGlass: false, isDrawer: true, hinges: 2 },
        { type: 'upper', hasHandle: false, hasGlass: true, isDrawer: false, hinges: 2 },
    ];

    // Викликаємо функцію calcFacadeOperations для обчислення операцій
    const operations = calcFacadeOperations(facades);
    console.log(operations);  // Перевірка результату
